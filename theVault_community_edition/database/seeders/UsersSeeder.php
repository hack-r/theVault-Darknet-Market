...
class UsersSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        for ($i = 0; $i < $this->numberOfAccounts; $i++) {
            ...
            $this->generateDepositAddressSeed($user);
            // Every fifth user is a vendor
            if ($i % 5 == 0) {
    ...
    public function generateAdminAccount()
    {
        ...
        $AdminEncryptedPrivateKey = \Defuse\Crypto\Crypto::encryptWithPassword($adminPrivateKey, $adminpassword);
    ...
    public function generateBuyerAccount()
    {
        ...
        $buyerEncryptedPrivateKey = \Defuse\Crypto\Crypto::encryptWithPassword($buyerPrivateKey, $buyerpassword);

        $buyer->msg_private_key = $buyerEncryptedPrivateKey;
        $buyer->msg_public_key = encrypt($buyerPublicKey);

        $buyer->save();

        $this->generateDepositAddressSeed($buyer);
