...
class ProductSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        for ($i = 0; $i <= $this->productsToCreate; $i++) {
            ...
            // Shipping
            $newShipping = new \App\Shipping;
            $newShipping->product_id = $newProduct->id;
            $newShipping->name = 'Shipping for ' . $newProduct->name;
            $newShipping->price = mt_rand() / mt_getrandmax() * 5; // max price is less than 5$
            $newShipping->duration = '1-2 weeks';
            $newShipping->from_quantity = 1;
            $newShipping->to_quantity = 30;
            $newShipping->save();
    ...
    /**
     * Check if there are X (where x is specified max amount) images in array, if there are return random image, if not create new image and add it to array
     *
     * @return mixed|string
     */
    private function getRandomImage()
    {
        ...
        if (count($this->imagesArray) < $this->maximumAmoutOfImagesToCreate) {
            // If there are less than X (where x is specified max amount) images, create new image and put name it in array
            $newImage = 'products/' . $faker->image(storage_path('app/public/products'), 256, 256, null, false);
            $this->imagesArray[] = $newImage;
            return $newImage;
        } else {
            // If there are X (where x is specified max amount) or more images
